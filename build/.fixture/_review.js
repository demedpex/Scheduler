...
/* 기간 업무 — 마감일자를 정하면 시작일부터 마감일까지 매일 목록에 보인다.
   ...
   완료는 "건 하나"에 붙는다. 날짜별로 따로 두지 않는다.
   중간에 한 번 체크하면 그 일이 끝난 것이고, 남은 날에도 완료로 보인다.
   (날짜마다 따로 체크하려면 doneDays 같은 목록이 필요한데, 그렇게 하지 않기로 정했다.
    "이 일을 끝냈다"가 사람이 실제로 뜻하는 바이고, 하루치 진척을 체크로 세지는 않는다.) */
